...
export const errorHandler = ({ dispatchApi, errorObject }) => {
    ...
    if (errorObject.graphQLErrors[0].extensions.exception.data) {
        errorMessage = errorObject.graphQLErrors[0].extensions.exception.data.data[0].messages[0].message ?? 'Unhandled error type.';
    } else {
        errorMessage = errorObject.graphQLErrors[0].extensions.exception.output.payload.message ?? 'Unhandled error type.';
    }
